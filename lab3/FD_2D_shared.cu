...
void test_shared(FILE *fp, int N, int BLOCKSIZE) {
    cudaSetDevice(0);
...
int main() {
    ...
    for (int N = 32; N <= 1024; N *= 2) {
        test_shared(fp, N, BLOCKSIZE);
    }
    test_shared(fp, 1536, BLOCKSIZE);
    test_shared(fp, 2048, BLOCKSIZE);
}

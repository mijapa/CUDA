...
void test_texture(FILE *fp, int N, int BLOCKSIZE) {
    ...
    int grid = ((N_max - 0.5) / BLOCKSIZE) + 1;
...
int main() {
    FILE *fp;
    if ((fp = fopen("texture.csv", "a")) == NULL) {
    ...
    int BLOCKSIZE = 5;    // 32. As I will be using BLOCKSIZE to be a multiple of 8
    // I'll just look for the closest multiple of BLOCKSIZE (N_max)


    test_texture(fp, N, BLOCKSIZE);
}
